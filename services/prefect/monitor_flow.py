...
@task
def decide_action(as_of_ref: str, as_of_cur: str, drift_share: float, target_drift: float,
                  threshold: float = 0.02) -> str:
    if drift_share >= threshold:
        print(f"[MONITOR] Drift detected ({drift_share:.2f} >= {threshold}). Triggering retraining...")

        # Appel direct du flow (Prefect gère la sous-exécution)
